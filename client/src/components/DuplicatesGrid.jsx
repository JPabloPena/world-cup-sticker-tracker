...
import { List, Copy, ChevronDown, ChevronUp, Home } from 'lucide-react';
...
export default function DuplicatesGrid() {
  ...
  const hasDuplicates = duplicatesData.length > 0;
      ...
      <div className={`mx-auto min-h-screen shadow-2xl ${darkMode ? 'bg-black/95' : 'bg-white/80'} backdrop-blur-xl`}>
        ...
        {!hasDuplicates ? (
          <div className={`flex flex-col items-center justify-center p-16 ${
            ...
            <Copy className="w-16 h-16 mb-4 opacity-50" />
            <p className="text-lg font-semibold">No hay duplicados</p>
            <p className="text-sm mt-2"> marca más de 1 para ver duplicados aquí</p>
          </div>
        ) : (
          <div className="p-4 space-y-2">
            {duplicatesData.map((country) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-sm ${darkMode ? 'text-green-600' : 'text-green-500'}`}>
                      ({country.duplicates.length} duplicados)
                    </span>
                  </div>
